    ...
    public function excel(Request $request)
    {
      if ($request->has('date') || $request->has('cabang') || $request->has('type') || $requets->has('status') ) {
        ...
        foreach ($getworker as $index => $work) {
          ...
          $image_before = $image_after = NULL;
          if ($work->img_before != NULL || $work->img_after != NULL) {
              $image_before = env('CDN_URL').'complaints_image/'.$work->img_before;
              $image_after = env('CDN_URL').'complaints_image/'.$work->img_after;
          }
          // $img_before =  Response::download(env('CDN_URL').'complaints_image/'.$work->img_before);
            $datacomplaint[] = [
              ...
              // 'img_before' => "<img src=".env('CDN_URL').'complaints_image/'.$work->img_before. '>',
            ];
        }
        // echo "<img src='/php_test/image_archive/" . $last_file[0] . "' alt='error'>";
        // return $datacomplaint;
        $data_complaint = 'Report_Complain_'.$request->date;
          Excel::create($data_complaint, function($excel) use ($datacomplaint) {
               $excel->sheet('sheet 1', function($sheet) use ($datacomplaint) {

                   $sheet->fromArray($datacomplaint,null,'A1',false,false)->prependRow(
                       array(
                           'Date', 'Time', 'Location','Type','Param','Status','Action Date','Start Date','End Date','Desc','Informer Param', 'Informer Name','Informer Department','Informer Position','Work','Solution','img before','img after'
                       )
                   );
               });
           })->download('xls');
